...
Experience Manager Assets 现在支持指定图像格式的 Content Credentials。 这提供了有关资产谱系及其创建方式的信息，包括是否使用 GenAI 进行了修改。
...
![存储使用情况洞察](/help/using/assets/contextual-search-text-prompt1.png)
...
Experience Manager Assets 现在包括由 Adobe Express 支持的简单而直观的视频编辑工具，可提高内容重用率，并加快内容创建速度。 编辑选项包括修剪、裁切、调整视频大小以及将 MP4 转换为 GIF 文件。
...
![存储使用情况洞察](/help/using/assets/block-tags.png)
...
Assets Essentials 现在支持使用“我的工作区”的&#x200B;**“洞察”**&#x200B;部分查看 Assets Essentials 部署中搜索最多的术语。 您还可以访问详细的洞察，以查看过去 30 天或 12 个月内的热门搜索。
...
“我的工作区”包括“洞察”、“任务”和“内容”小组件。 可根据自己的偏好配置如何在工作区中显示这些小组件。
...
现在，一旦您选择使用 Assets Essentials 用户界面共享资源，Assets Essentials 即自动生成链接。 即使您更改过期日期，所生成的链接仍保持有效。
...
通过资源报表，管理员可评估 Adobe Experience Manager Assets Essentials 中的用户活动。 “报表和实时统计数据”仪表板提供关于用户如何与可在您的部署中找到的资源进行交互的有用信息。[使用报表](manage-reports.md)中的信息获得关键成功指标以衡量在您的企业内和客户采用Assets的情况。
...
* 现在通过 Assets Essentials 可[设置资源的过期日期](manage-organize.md#set-asset-status)。 此外，还可根据 `Expired` 资源状态和有效期限日期范围[筛选资源](search.md#refine-search-results)。
...
* 现已提供 [Assets Essentials 概述文档](introduction.md)的新版本。
...
* [!DNL Assets Essentials] 现在支持[创建收藏集](manage-collections.md)。 收藏集是 Experience Manager Assets Essentials 中的一组资源。 使用收藏集可在用户之间共享资源。 收藏集与文件夹的不同之处是可包含来自不同位置的资产。
...
* [!DNL Assets Essentials]现在使您能够[生成链接并与无权访问[!DNL Assets Essentials]应用程序的外部利益相关者](share-links-for-assets.md)共享资源。 您可以定义链接的过期日期，然后使用您喜欢的通信方式（如电子邮件或消息服务）与他人共享。 链接的接收者可以预览并下载资源。
...
* 多个错误修复和产品增强功能，包括以前的已知问题（现在文件夹在上传<!-- CQ-4337638 -->之后正确显示在左导航树中，执行拖放上传操作时，用户可在执行放置操作时选择当前文件夹或者任意子文件夹）。
