...
随着时间的推移，受控词汇的扁平列表变得难以管理。 管理员能够使用分层的标记结构，该结构便于应用相关的元数据、为资源分类、支持搜索、重用标记、提高可发现性等。
...
* 分层标记很灵活，能够很好地以逻辑方式组织术语。 通过命名空间、标记和子标记，可以表示整个分类系统。

* 随着组织词汇表的变化，标记可能会逐渐发生演变。

* 在 Assets as a Cloud Service 中管理的标记与在 Assets Essentials 中管理的标记保持同步，从而确保元数据得到治理及其完整性。

为了可将标记应用于资源，必须首先创建命名空间，然后创建标记并将其添加到该命名空间。 您还可以创建标记并将其添加到现有命名空间。 您在根级别创建的任何标记都会自动添加到标准标记命名空间。 然后，您可以将“标记”字段添加到元数据表单，以便它显示在“资源详细信息”页面上。 配置这些设置后，您可以开始将标记应用到资源。
...
标记组件会自动添加到 `default` 元数据表单中。 您可以使用模板或从头开始设计[元数据表单](https://experienceleague.adobe.com/docs/experience-manager-assets-essentials/help/metadata.html?lang=en#metadata-forms)。 如果您使用的不是现有的元数据表单模板，则可以修改您的元数据表单并添加标记组件。 元数据属性映射会自动填充，此时无法修改。[!DNL Assets as a Cloud Service] 用户可以更新映射以使用自定义命名空间存储标记值，并使用根路径仅公开层次结构的子集。
...
通过 [!DNL Assets Essentials] 可配置一个阻止列表，其中包括在将资源上传到存储库时不应作为智能标记添加到资源的字词。 此功能帮助您确保品牌合规，并减少审核智能标记的工作量。
...
通过 [!DNL Assets Essentials]，管理员可为现有资源和新添加的资源阻止智能标记。 要阻止标记，请执行以下步骤：
...
通过 Assets Essentials 可将列出的阻止的标记导出为 CSV 格式。 要导出阻止列表，请执行以下步骤：
...
通过 Assets Essentials 可从数据源 (CSV) 导入阻止的标记。 要导入阻止列表，请执行以下步骤：
